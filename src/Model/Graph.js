export default class Graph {
    ...
    counterclockwise(x, y, z) {
        return (y.xPos - x.xPos) * (z.yPos - x.yPos) - (z.xPos - x.xPos) * (y.yPos - x.yPos);
    }
    ...
    calculateConvexHull() {
        ...
        let p0 = points.shift()
        let sortedPoints = this.sortVerticesByPolarAnglesWithVertex(p0, sortedPoints)
        sortedPoints.unshift(p0)
        let stack = []
        for (point in sortedPoints) {
            while ((stack.length > 1) && this.counterclockwise(stack[stack.length - 2], stack[stack.length - 1], point) <= 0) {
                stack.pop()
            }
            stack.push(point)
        }
